...
from sensor_manifest_lib import ManifestError, fixture_manifest, load_manifest, load_manifest_v2, manifest_v2
...
def render_gateway_manifest_header(sensors: List[Dict[str, str]], gateway_meta=None, history_meta=None) -> str:
    manifest_json = json.dumps(
        manifest_v2(sensors, f"v{VERSION}", gateway_meta=gateway_meta, history_meta=history_meta),
        indent=2,
        ensure_ascii=False,
    )
    return (
...
def render_yaml_file(path: Path, sensors: List[Dict[str, str]]) -> str:
    ...
    text = re.sub(r'register_history_handler\((.*?), "v[0-9.]+"\);', lambda m: f'register_history_handler({m.group(1)}, "v{VERSION}");', text, count=1)
    text = re.sub(r'(live dashboard\. v)[0-9]+\.[0-9.]+', lambda m: m.group(1) + VERSION, text)
...
def main() -> int:
    ...
    # Load gateway/history metadata from v2 manifest if available
    gateway_meta = None
    history_meta = None
    try:
        v2_data = load_manifest_v2(MANIFEST_PATH)
        gateway_meta = v2_data.get("gateway")
        history_meta = v2_data.get("history")
    ...
    expected_gateway_manifest = render_gateway_manifest_header(sensors, gateway_meta=gateway_meta, history_meta=history_meta)
    expected_fixture_sensors = json.dumps(fixture_manifest(sensors), indent=2) + "\n"
    expected_fixture_manifest = json.dumps(manifest_v2(sensors, f"v{VERSION}", gateway_meta=gateway_meta, history_meta=history_meta), indent=2, ensure_ascii=False) + "\n"
