...
GATEWAY_MANIFEST_PATH = ROOT / "dashboard" / "gateway_manifest.h"
...
def render_yaml_file(path: Path, sensors: List[Dict[str, str]]) -> str:
    ...
    # Update version in dashboard_link publish_state text (format: "... vX.Y.Z: ...")
    text = re.sub(r'(\.publish_state\([^"]*"[^"]*?\s)v[0-9]+(?:\.[0-9]+)+:', lambda m: m.group(1) + f"v{VERSION}:", text)
    # Update version in YAML comments referencing the repo version
    text = re.sub(r'(#[^\n]*?)\bv[0-9]+\.[0-9]+\.[0-9]+(?:\.[0-9]+)?\b', lambda m: re.sub(r'v[0-9]+\.[0-9]+\.[0-9]+(?:\.[0-9]+)?', f"v{VERSION}", m.group(0)), text)
...
def render_gateway_manifest(sensors_v2: List[Dict], gateway_meta: Dict, history_meta: Dict) -> str:
    """Generate the dashboard/gateway_manifest.h C header with a static JSON string."""
    manifest_dict = manifest_v2(sensors_v2, VERSION, source="firmware",
                                gateway_meta=gateway_meta, history_meta=history_meta)
...
def main() -> int:
    ...
    try:
        manifest_data = load_manifest_v2(MANIFEST_PATH)
    except ManifestError as exc:
        print(f"Manifest error: {exc}", file=sys.stderr)
        return 2

    sensors = manifest_data["sensors"]
    sensors_v2 = manifest_data["sensors_v2"]
    gateway_meta = manifest_data["gateway"]
    history_meta = manifest_data["history"]
    ...
    expected_fixture_sensors = json.dumps(fixture_manifest(sensors), indent=2, ensure_ascii=False) + "\n"
    expected_fixture_manifest = json.dumps(
        manifest_v2(sensors_v2, VERSION, gateway_meta=gateway_meta, history_meta=history_meta),
        indent=2,
        ensure_ascii=False,
    ) + "\n"
    ...
    expected_gateway_manifest = render_gateway_manifest(sensors_v2, gateway_meta, history_meta)
    ...
    expected = {
        ...
        GATEWAY_MANIFEST_PATH: expected_gateway_manifest,
    }
