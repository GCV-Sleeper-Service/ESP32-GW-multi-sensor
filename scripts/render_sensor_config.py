...
from sensor_manifest_lib import ManifestError, fixture_manifest, load_manifest, manifest_v2, to_manifest_v2
...
def render_gateway_manifest_h(sensors: List[Dict[str, str]], version: str) -> str:
    """Generate dashboard/gateway_manifest.h with the v2 manifest as a C string literal."""
    manifest = to_manifest_v2(sensors, version)
    manifest_json = json.dumps(manifest, indent=2)
    ver_tag = version if version.startswith("v") else f"v{version}"
    return (
        '#pragma once\n'
        '// gateway_manifest.h \u2014 generated v2 manifest JSON\n'
        f'// Auto-generated by scripts/render_sensor_config.py {ver_tag} \u2014 do not edit\n'
        '\n'
...
def main() -> int:
    ...
    expected_fixture_manifest = json.dumps(manifest_v2(sensors, f"v{VERSION}"), indent=2) + "\n"
    ...
    expected_gateway_manifest_h = render_gateway_manifest_h(sensors, f"v{VERSION}")
    ...
    expected = {
        ...
        GATEWAY_MANIFEST_PATH: expected_gateway_manifest_h,
    }
