 ...
 public:
  ...
  bool canHandle(AsyncWebServerRequest *request) const override {
    ...
    if (request->method() == HTTP_POST) {
      ...
      // Reject zero-length POSTs for management routes, but keep import URL POST
      // compatibility (history_backup.py uses POST with URL payload and empty body).
      if (request->contentLength() == 0 && is_management_post_route_(p)) return false;
      return is_post_or_options_route_(p);
    }
