...
function buildMergedSensorCsv(meta, sensors, sensorRowsList) {
  ...
  sensorRowsList.forEach(function(rows, idx) {
    if (!rows) return; // BUG-046: skip sensors with no data after migration/failure
    rows.forEach(function(row) {
...
function normalizeManifestSensors(payload) {
  ...
  return sensors.map(function(sensor) {
    ...
    // Only include environmental sensors in SENSORS — network/system devices have
    // no card renderer until their respective phase (v7.5.4.2+).
    return sensor.id && sensor.name && (!sensor.category || sensor.category === 'environmental');
  });
}
...
// Environmental (ThermoPro) is the only category for this step.
