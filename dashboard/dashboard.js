...
function _handleTestSatellite(urlInput, statusEl) {
  ...
  requestManagementCredentials('satellite test')
    ...
    .catch(function(err) {
      if (statusEl) statusEl.textContent = '\u2717 ' + escHtml(err.message || 'Test failed');
    })
...
function _handleAddSatellite(urlInput, nameInput, statusEl) {
  ...
  .catch(function(err) {
    if (statusEl) statusEl.textContent = '\u2717 ' + escHtml(err.message || 'Add failed');
  })
  ...
  requestManagementCredentials('satellite removal')
    ...
    .catch(function(err) {
      ...
      if (msg.indexOf('404') !== -1) {
        ...
        if (statusEl) statusEl.textContent = '\u2717 ' + escHtml(msg);
      }
    })
