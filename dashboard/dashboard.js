...
function _handleTestSatellite(urlInput, statusEl) {
  ...
  // Capture values synchronously before any async work (Bug 2 fix)
  var capturedUrl = String(urlInput ? urlInput.value || '' : '').trim();
  var capturedStatusEl = statusEl;
  if (!capturedUrl) {
    if (capturedStatusEl) capturedStatusEl.textContent = 'Enter a URL to test.';
    return;
  }
  _satTestInFlight = true;
  if (capturedStatusEl) capturedStatusEl.textContent = 'Authenticating...';
  requestManagementCredentials('satellite test')
    .then(function(creds) {
      if (!creds) {
        if (capturedStatusEl) capturedStatusEl.textContent = 'Test cancelled';
        return null;
      }
      if (capturedStatusEl) capturedStatusEl.textContent = 'Testing...';
      ...
      .then(function(data) {
        ...
        if (capturedStatusEl) capturedStatusEl.innerHTML = '\u2713 Found: ' + escHtml(gw.name || '\u2014') +
          ' (' + escHtml(gw.hardware || '\u2014') + ', ' + escHtml(deviceCount) + ' devices)';
      });
    })
    .catch(function(err) {
      if (capturedStatusEl) capturedStatusEl.innerHTML = '\u2717 ' + escHtml(err.message || 'Test failed');
    })
