...
function renderAllGatewaysSummary(gateways) {
  ...
  gateways.forEach(function(gw) {
    ...
    html += '<div class="gw-summary-header"><span class="' + statusClass + '">' + statusText + '</span>' +
      ...
      ' <span class="gw-summary-id">(' + gw.id + ')</span></div>';
    html += '<div class="gw-summary-details">';
    html += '<div><span class="gw-detail-label">Last seen:</span> ' + lastSeenStr + '</div>';
    html += '<div><span class="gw-detail-label">Firmware:</span> ' + fwVersion + '</div>';
    html += '<div><span class="gw-detail-label">Devices:</span> ' + deviceCount + '</div>';
...
function renderGatewayDevices(gwId) {
  ...
  grid.innerHTML = '';
  if (!gw) {
    grid.innerHTML = '<div class="gw-summary-card gw-stale"><div>Gateway not found: ' + gwId + '</div></div>';
    return;
  }
  if (!gw.reachable) {
    ...
    grid.innerHTML = '<div class="gw-summary-card gw-stale">' +
      ...
      '<div>Last seen: ' + lastSeenStr + '</div></div>';
    return;
  }
  ...
  if (!manifest || !manifest.sensors || manifest.sensors.length === 0) {
    grid.innerHTML = '<div class="gw-summary-card"><div>No device data available for <strong>' +
      gw.name + '</strong>. Manifest not yet cached.</div></div>';
    return;
  }
  ...
  manifest.sensors.forEach(function(s, idx) {
    ...
    // Store originals for proxy history routing
    cfg._gwId = gwId;
    cfg._deviceId = s.id;
...
function renderSettingsPanel(gateways) {
  ...
  gateways.forEach(function(gw) {
    ...
    html += '<div class="settings-sat-header">' + statusDot + ' ' + gw.name +
      ' <span class="settings-sat-id">(' + gw.id + ')</span></div>';
...
App.Boot.start = function() {
  ...
  detectAggregatorMode().then(function(isAggregator) {
    if (isAggregator) {
      // Aggregator boot path: load manifest, build local device cards if any, init aggregator UI
