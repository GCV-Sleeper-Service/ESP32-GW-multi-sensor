...
- curl http://192.168.120.189/api/v2/live — verify valid JSON with current sensor values
...
3. curl /sensors.json — verify v1 compatibility response
4. curl /api/manifest — verify v2 manifest
5. curl /api/status — verify health, check free heap
6. curl /api/v2/live — verify live values
7. curl /history/office/temp — verify legacy history endpoint
8. curl /api/v2/history/office/temp — verify v2 history endpoint
