...
1. Compile-time constants and standard includes (lines 1–124)
2. `HistEntry`, `HistoryBuffer`, `MetricDef`, `MetricState`, `SensorEntity` types (lines 126–369)
...
│   │   ├── config.h                    ← lines 1–124 (compile-time constants, standard includes)
│   │   ├── data-model.h                ← lines 126–525 (types + ENTITY marker blocks)
...
|---|---|---|---|
| `config.h` | 1–124 | YES | Standard includes, `TAG` (line 96), and compile-time `#define`/`constexpr` configuration through line 124 — no consumers, only providers. Zero runtime symbols. Cleanest possible extraction seam. |
| `data-model.h` | 126–525 | YES | Begins at `HistEntry` (line 126) and contains `HistoryBuffer`, all metric/entity type definitions, and both generator marker blocks. Everything downstream depends on this module; nothing in this block depends on anything downstream. Marker blocks must live here so the generator writes into the assembled artifact's type section. |
...
| File | Change |
|---|---|
| `scripts/provision.sh` | Add `--auto` flag and `pipeline` subcommand that executes all 9 pipeline steps automatically; add `--dry-run` option; add dependency checks for `node`, `npm`, `python3`, `gzip` |
...
# New behavior:
provision.sh <target> --auto        # switch config + execute all 9 steps automatically
...
- [ ] `provision.sh <target> --auto` executes all 9 steps in order, exits 0 on success
- [ ] `provision.sh <target> --dry-run` prints all 9 steps without executing any
...
At v7.6.6.1 the `MODULES` array lists fragment paths that do not yet exist. The script implements **fixed-range splicing**: it maintains a range map (which line range each fragment covers), and for each `--write` invocation replaces those line ranges with fragment file content while preserving all other line ranges byte-for-byte from the current `dashboard/sensor_history_multi.h`. When no fragments exist, the script reads all lines from the existing assembled file — output is identical to input, SHA-256 unchanged.

**Acceptance criteria:**

- [ ] `firmware/core/` directory exists in repository
- [ ] `scripts/assemble-firmware-modules.sh` exists and is executable
- [ ] `assemble-firmware-modules.sh --check` exits 0 when all listed fragments are absent (all ranges read from existing assembled file — identity pass)
- [ ] `assemble-firmware-modules.sh --dry-run` prints assembly order and range map
- [ ] `assemble-firmware-modules.sh --write` with no fragments present produces no change to `sensor_history_multi.h` (SHA-256 unchanged — confirmed by identity pass of fixed-range splicing)
...
**Lines extracted:** 1–124 (compile-time constants, standard includes, `TAG` definition, and all compile-time `#define`/`constexpr` configuration through `AUTH_REALM`)

**Exact boundary:** Line 1 (`#pragma once`) through line 124 (blank line following `AUTH_REALM` constant). `TAG` is defined at line 96; compile-time constants (`HISTORY_HOURS`, `HISTORY_INTERVAL_MINUTES`, `PERSIST_DAYS`, `HISTORY_POINTS_PER_SERIES`, `HISTORY_META_MAGIC`, `HISTORY_META_VERSION`, `HISTORY_SERIES_TEMP`, `HISTORY_SERIES_HUM`, `AUTH_FAILURE_DELAY_MS`, `AUTH_LOCKOUT_MS`, `AUTH_MAX_FAILURES`, `AUTH_REALM`) span lines 98–123.

**Files created / modified:**

| File | Change |
|---|---|
| `firmware/core/config.h` | New fragment: lines 1–124 of current `sensor_history_multi.h` |
| `dashboard/sensor_history_multi.h` | Regenerated by fixed-range splicing: lines 1–124 replaced from `firmware/core/config.h`; all other line ranges preserved byte-for-byte from existing file |
| `scripts/assemble-firmware-modules.sh` | `config.h` slot now populated; `--check` verifies lines 1–124 of assembled file match fragment content |

**Acceptance criteria:**

- [ ] `firmware/core/config.h` exists and contains lines 1–124 (verified by `grep -n '#pragma once' firmware/core/config.h` → line 1)
- [ ] `firmware/core/config.h` ends with `AUTH_REALM` constant (line 122–123) followed by blank line
...
**Lines extracted:** 126–525 (`HistEntry`, `HistoryBuffer`, `MetricDef`, `MetricState`, `SensorEntity` type definitions + both `SENSOR_MANIFEST:HEADER` and `SENSOR_MANIFEST:ENTITY` marker blocks + end-of-generated-block comments)

**Exact boundary:**
- Start: line 126 (`struct HistEntry {` — first type definition, immediately after the section comment at line 125)
...
| File | Change |
|---|---|
| `firmware/core/data-model.h` | New fragment: types + marker blocks (lines 126–525) |
...
| v7.6.6.2 | Extract `config.h` (lines 1–124) | Low | 0.5 sessions |
| v7.6.6.3 | Extract `data-model.h` (lines 126–525, includes generator marker blocks) | High | 1 session |
...
|---|---|
| v7.6.6.0 → v7.6.6.1 | `provision.sh --auto` executes all 9 steps successfully in a clean environment |
...
### Current Route Inventory (v2 inventory §5 — 27 routes in 3 families)
...
All 27 routes remain owned by `HistoryWebHandler` in `web-handler.h`. No route moves to another fragment.
...
| `tests/mock-server/server.js` | Contract-faithful implementation of all 27 routes | All route families |
...
The current Playwright suite covers the full HTTP API surface (all 27 routes across 3 families). Because Phase Y makes no behavioral changes, the existing test suite is **sufficient** to verify Phase Y correctness for every step.
