...
**Completed:** Phase 1 completion — manifest v2 full schema, gateway_manifest.h, sensor config v2

- Upgraded `config/sensors.json` to full v2 schema with `gateway`, `history`, and per-sensor `measurements` blocks (id/name/mac retained for backward compat)
- Created `config/sensors.v2.example.json` as a reference template for the v2 schema
- Extended `scripts/sensor_manifest_lib.py` with new functions:
  - `to_manifest_v2()` — promotes a v1 sensor list to full v2 manifest with ThermoPro defaults
  - `validate_v2_schema()` — validates v2 manifest required fields
  - `load_manifest_v2()` — returns full v2 manifest dict (preserves gateway/history/measurements)
  - `save_manifest()` — writes sensor list back, preserving gateway/history blocks (needed by `change_sensor_number.py`)
  - `validate_sensors()` — validation alias used by `change_sensor_number.py`
  - `load_manifest()` now accepts both v1 and v2 formats
- Upgraded `manifest_v2()` to include `gateway`, `history`, per-metric `class`, `data_type`, `history`, and `display` fields
- Added `dashboard/gateway_manifest.h` — compile-time static C string holding the full v2 manifest JSON, generated by `render_sensor_config.py`
- Replaced inline `handle_api_manifest_()` (single-line `resp->print()` chain) with a clean implementation that serves `GATEWAY_MANIFEST_JSON` from `gateway_manifest.h`
- Fixed compressed/minified formatting in `sensor_history_multi.h`: `canHandle()` route check and `handleRequest()` dispatch for `/api/manifest` are now properly multi-line
- Added `manifest -> GET /api/manifest` to `register_history_handler()` startup log
- Aligned `tests/fixtures/manifest.json` with full v2 schema (gateway, history, class, data_type, display fields)
- Updated `tests/fixtures/generate-fixtures.js` to produce identical v2 schema output to Python `manifest_v2()` (eliminates dual-ownership fixture drift)
- Updated `scripts/preflight.sh`:
  - Removed `apply_phase1_manifest_patch.py` from required files (script deleted)
  - Added `dashboard/gateway_manifest.h` to required files check
  - Added `gateway_manifest_header_exists` and `gateway_manifest_uses_include` checks
  - Added `config_sensors_schema_v2` check (schema_version: 2 in sensors.json)
  - Added `version_anti_regression` check (VERSION file matches render_sensor_config.py VERSION constant)
  - Integrated ESPHome config parse check inline (replaces `preflight-esphome-parse.sh`)
- Deleted dead scripts: `scripts/apply_phase1_manifest_patch.py` and `scripts/preflight-esphome-parse.sh` (functionality now integrated)
- Added `tests/playwright-results/` and `tests/playwright-report/` to `.gitignore`
- Version: `v7.5.1`

**Validated:**
- `python3 scripts/render_sensor_config.py --check` → PASS
- `bash scripts/preflight.sh` → all applicable checks PASS
- All 38 Playwright browser tests pass (2 pre-existing DNS resolution failures in sandbox unrelated to firmware changes)
- Python and JS fixture generators produce byte-identical `manifest.json` (dual-ownership resolved)

**Documentation:**
- `Docs/changelog.md` — v7.5.1 entry added
- `Docs/bugs-and-lessons-learned.md` — BUG-040, BUG-041, BUG-042, LESSON-OPS-046–048 added
- `Docs/session-log-2026-03-15-phase1-completion.md` — session log created
