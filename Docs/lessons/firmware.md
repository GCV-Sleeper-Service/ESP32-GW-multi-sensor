...
### BUG-084: Heap exhaustion under concurrent HTTP connections crashes non-PSRAM boards (2026-05-05)

The `scripts/stress-test-httpd-stack.sh` script sends 8 concurrent HTTP requests per wave
(mix of `/api/status/full`, `/api/storage-stats`, and `/history/` endpoints). This crashes
the C3 and WROOM boards via heap exhaustion during Wave 1–2. The S3 aggregator survives
because PSRAM absorbs the connection buffer pressure.

**Symptoms:**
- C3 (192.168.120.189): Crash during Wave 2. Pre-stress `min_free_heap`=47,616 B drops to
  19,956 B after Wave 1. Wave 2 pushes below WiFi/LWIP minimum, triggering ESP-IDF panic.
  Board reboots (uptime resets to 81 s).
- WROOM (192.168.120.170): Crash during Wave 1. Pre-stress `min_free_heap`=13,216 B (already
  low after 8 days of operation). 8 concurrent connections immediately exceed remaining heap.
  Board reboots (uptime resets to 247 s).
- S3 (192.168.120.191): Board survives (uptime unchanged at 707,016 s). The stress test
  *script* exits prematurely because `set -euo pipefail` + backgrounded `curl -sf` returning
  non-zero on a single timed-out request kills the whole script.

**Root cause:** Each concurrent HTTP connection requires ~3–5 KB of heap for TCP socket
buffers, HTTP header parsing, JSON serialization, and response data. 8 simultaneous
connections on a board with 58 KB (C3) or 35 KB (WROOM) free heap exceeds the safe minimum
needed for WiFi/LWIP operation (~15–20 KB on ESP32-C3/ESP32).

**This is NOT a stack overflow.** The httpd stack watermarks remained healthy throughout:
C3=12,900 B, WROOM=12,996 B (on the 16 KB stack). The 16 KB stack override (BUG-076 fix)
is working correctly. The crash is a separate resource limit.

**This is NOT a regression from v7.6.10.0 or ESPHome 2026.4.1.** The same crash would have
occurred on v7.6.9.5 — the stress test was designed to verify stack watermarks, not to test
maximum concurrency. Previous phases never ran the full 5-wave stress test on the C3 or WROOM.

**Impact:** Any scenario where 6+ simultaneous HTTP connections hit a non-PSRAM board can
crash it. In practice, this means opening the dashboard in multiple browser tabs, or having
the aggregator polling while a user opens the dashboard, could trigger a crash on C3/WROOM.

**Affected boards:** All non-PSRAM boards (C3, WROOM, and future C6). PSRAM-equipped boards
(S3 DevKitC 8 MB, S3 SuperMini 2 MB, C5 WROOM-1U 8 MB) are unaffected.

**Recommended fixes:**

1. **Stress test script** (`scripts/stress-test-httpd-stack.sh`):
   - Add `--concurrent=N` parameter (default 4, was 8)
   - Replace `curl -sf` with `curl -s --max-time 10` and check status individually
   - Remove reliance on `set -e` killing the script when one of 8 background curls fails
   - Keep the watermark acceptance gate at ≥ 10,000 B

2. **Documentation**: Record the concurrency limits per board class:
   - No PSRAM (C3, WROOM, C6): safe concurrent connections ≤ 4
   - With PSRAM (S3, C5): safe concurrent connections = CONFIG_LWIP_MAX_SOCKETS

3. **Future (Phase 7)**: Consider reducing `CONFIG_LWIP_MAX_SOCKETS` from 15 to 8 on
   non-PSRAM satellite boards. This would cause the httpd to refuse excess connections
   at the TCP layer (connection refused) rather than accepting them and crashing from
   heap exhaustion. Trade-off: aggressive clients get HTTP 503 instead of a board crash.

**Status:** OPEN. Stress test script fix planned for v7.6.10.1 or pre-v7.6.10.2 operator task.

---

### LESSON-OPS-129: Agent prompts must distinguish directly-modified files from pipeline-regenerated files (2026-05-05)

Context: PR #200 (v7.6.10.0). The session handoff listed "Files modified: 4 files" but the
PR touched 38 files. The discrepancy confused the review process and caused concern about
scope creep.

The root cause: a version bump in `render_sensor_config.py` triggers the full regeneration
pipeline (Critical Rule 37), which updates ~20+ generated files: `dashboard.js`,
`dashboard.html`, `dashboard.h`, `sensor_history_multi.h`, `gateway_manifest.h`,
`aggregator_config.h`, all fixture manifests, variant manifests, firmware YAML version
strings, and firmware core header version comments.

**Rule:** Every session handoff and agent prompt that includes a VERSION bump must have
two separate file lists:
- "Files directly modified by this step" — the actual implementation changes
- "Files regenerated by pipeline (expected, not scope creep)" — the cascade from
  `provision.sh` / `render_sensor_config.py --write` / `generate-fixtures.js` etc.

This separation prevents agents from being confused about scope and prevents reviewers
from questioning whether the change count is warranted.

---

### LESSON-OPS-130: patch-esphome-httpd-stack.sh --check drift warning is expected when local override has project-specific fixes (2026-05-05)

Context: v7.6.10.0 post-merge verification. Running `patch-esphome-httpd-stack.sh --check`
reported "WARNING: Upstream web_server_idf.cpp has diverged (46 line differences)."

The `--check` mode's drift detection strips only the two known patch lines
(`config.stack_size = 16384` and the `PATCH2-BEGIN...PATCH2-END` block) before comparing
the local override against the installed upstream. But PR #200's review-fix commit added
additional project-specific changes:
- Restored 16 explicit HTTP status code mappings (fixing a review-caught regression)
- Added `#if ESPHOME_VERSION_CODE` compatibility gate for the SSE callback type
- Changed the `default:` case in `init_response_()` from `snprintf` to `HTTPD_500`

These are legitimate project customizations that the drift detection doesn't know about.
The 46-line divergence is correct and expected.

**Rule:** Do NOT re-run `patch-esphome-httpd-stack.sh` without `--check` to "fix" the
drift warning. Doing so would overwrite the local override with a fresh upstream copy and
re-apply only the two standard patches — losing the HTTP status mappings and the version
gate. Only re-run without `--check` after a deliberate ESPHome upgrade, and then immediately
verify that all project-specific fixes are preserved.

**Future improvement:** The drift detection should also strip the status mapping block and
the version gate before comparing, to reduce false-positive warnings.

---

### LESSON-OPS-131: ESP32 IRAM optimization (sram1_as_iram) is counterproductive when heap is the bottleneck (2026-05-05)

Context: ESPHome 2026.4.1 emits a new boot-time hint on the ESP32 (WROOM-32D):
"Bootloader supports SRAM1 as IRAM (+40KB). Set sram1_as_iram: true under esp32 > framework > advanced"

Analysis of whether to enable this for the project's boards:

**ESP32/WROOM (Xtensa LX6, dual-core):**
`sram1_as_iram: true` remaps ~40 KB of SRAM1 from DRAM to IRAM. This gives more
instruction RAM for code execution but subtracts ~40 KB from the heap pool. The WROOM
already has the tightest heap in the fleet (34 KB free_heap, crashes under 8 concurrent
HTTP connections per BUG-084). Enabling this would worsen heap exhaustion. **Do NOT enable.**

**ESP32-S3 (Xtensa LX7, dual-core):**
Build output shows `IRAM: 100.0% (16384/16384)` — IRAM is fully consumed. However, the
S3 has 8 MB PSRAM and 16 MB flash. Code that exceeds IRAM simply executes from flash
(slightly slower, functionally identical). The S3 has no heap pressure (8+ MB min_free_heap
including PSRAM). The `sram1_as_iram` flag does not exist for the S3 — its memory architecture
handles IRAM overflow automatically. **No action needed.**

**ESP32-C3/C6/C5 (RISC-V):**
RISC-V chips have a unified memory architecture — there is no separate IRAM/DRAM split.
All RAM is accessible as both instruction and data memory. The `sram1_as_iram` concept
does not exist for these chips. **Not applicable.**

**Conclusion:** No board profile changes needed. No further investigation needed for C5/C6
boards on this matter. The IRAM optimization is a trade-off that hurts this project's
primary constraint (heap headroom on non-PSRAM boards).
