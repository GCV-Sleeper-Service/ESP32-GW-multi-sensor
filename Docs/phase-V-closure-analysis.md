...
**V2-E history auth note:** V2-E shipped auth guards on both history handlers (v7.6.8.1). The v7.6.9.0 PR (#183) audit (FINDING-V769-04) documents that during V3-A development, the history endpoints were found to not have `authenticate_management_()` — the audit classified this as a deliberate trade-off ("read-only history is exposed publicly for operational convenience") accepted by Copilot review. `SEC-ADR-001` was not updated. The current code (post-v7.6.9.5) confirms: neither `handle_history_()` nor `handle_api_v2_history_()` appear among the 16 auth-gated handler call sites (verified by grep against `firmware/core/web-handler.h`). This is a code-vs-ADR mismatch tracked in Q2.
