...
### BUG-042: `dashboard/dashboard.h` version not explicitly checked in preflight (post-v7.5.2.0)

**Symptom:** If `render_sensor_config.py --write` is run after a version bump but `generate-header.sh` is not run, `dashboard/dashboard.h` retains the stale embedded version. Preflight passed (checking `dashboard.js` App.version) while the header actually flashed to the device contained the old version.

**Root cause:** `scripts/render_sensor_config.py --write` updates `dashboard/dashboard.js` but does not update `dashboard/dashboard.h`. That header is a separate generated artifact produced only by `bash scripts/generate-header.sh`. Preflight checked `dashboard.js` version but not `dashboard.h` version, so a missing `generate-header.sh` step went undetected.

**Fix:** Added `dashboard_h_version_matches` check to `scripts/preflight.sh` that explicitly verifies `dashboard/dashboard.h` contains `App.version = '${VER_TAG}'`. Also added `render_sensor_config_py_version_sync` check to detect drift in `scripts/render_sensor_config.py` VERSION constant before the generator sync check runs. Added `scripts/bump-version.sh` to automate all version bump steps atomically.
...
**Enforcement:** Preflight now includes `dashboard_h_version_matches` (detects missing `generate-header.sh`) and `render_sensor_config_py_version_sync` (detects missing `render_sensor_config.py` VERSION update) in addition to the existing `fixture_generator_version_sync` and `render_sensor_config --check`.
