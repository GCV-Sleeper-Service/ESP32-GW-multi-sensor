...
_Last updated: 2026-03-15 — v7.5.1 (BUG-040, LESSON-OPS-046–047 added)_
...
### BUG-040: Dead patch script listed in REQUIRED_FILES caused preflight failure after script removal (v7.5.1)

**Symptom:** After `scripts/apply_phase1_manifest_patch.py` was identified as dead code and scheduled for removal, `bash ./scripts/preflight.sh` would fail immediately with "Missing scripts/apply_phase1_manifest_patch.py" because the REQUIRED_FILES list still referenced it.

**Root cause:** The REQUIRED_FILES list in `preflight.sh` was not updated in sync with file additions and removals. Dead scripts that have been superseded by better approaches (e.g., generator-based workflow) should be removed from REQUIRED_FILES at the same time they are deleted.

**Fix:** Removed `scripts/apply_phase1_manifest_patch.py` and `scripts/preflight-esphome-parse.sh` from REQUIRED_FILES in `preflight.sh`, added `dashboard/gateway_manifest.h` and `config/sensors.v2.example.json`, and deleted the dead files.
...
### LESSON-OPS-047: Generated files that are the canonical test fixture must be regenerated by --write before --check (v7.5.1)

When `render_sensor_config.py` owns files that are also regenerated by separate tools (e.g., `generate-fixtures.js`), running the second tool after `--check` leaves the file in a slightly different state. This causes `--check` to fail on the next run until `--write` is run again. The intended workflow is always: `--write` first, then `--check`, then the secondary tool. Document this in session setup notes and never run `--check` as a standalone gate without a preceding `--write`.

---

### LESSON-OPS-046: REQUIRED_FILES in preflight.sh must be updated in the same commit as file additions and removals (v7.5.1)

When a file is deleted or a new file is added to the project, `REQUIRED_FILES` in `preflight.sh` must be updated in the same commit. Keeping them in sync prevents "Missing <file>" preflight failures that block the workflow immediately after a file move or removal.

Pattern: whenever a script is identified as dead code and scheduled for deletion, add a TODO comment next to its REQUIRED_FILES entry, and handle both in the same PR.
